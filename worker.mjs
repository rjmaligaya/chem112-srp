export default {
  async fetch(request, env, ctx) {
    ...
    for (const t of body.trials) {
      // For meta rows, store estimate in 'estimate' column; otherwise blank
    ...
    try {
      ...
      if (head) {
        return new Response(JSON.stringify({ ok: true, saved: false, reason: "already_exists", key }), {
          status: 200,
    ...
    return new Response(JSON.stringify({ ok: true, saved: true, key }), { status: 200, headers: { ...corsHeaders(request), "Content-Type": "application/json" } });
  }
}
